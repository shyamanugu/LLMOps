...
- **Encryption at rest:** Platform-managed keys by default with **customer-managed keys (CMK)** in Key Vault for Tier 3 data stores (transcripts, candidate data, payment-adjacent records).

## Network Isolation

- All platform services (AI Foundry, Azure OpenAI, AI Search, Cosmos DB, Key Vault, Storage) are deployed into a **hub-and-spoke VNet** and reachable only through **private endpoints**.
...
- **Data residency:** Afni's footprint spans the US, Mexico, and the Philippines. Regional Azure deployments and data-boundary configuration keep regulated data (PHI, cardholder data, candidate PII) resident in the appropriate jurisdiction; model endpoints are pinned to compliant regions and data zones. Cross-border flows are governed by Purview policy and contractual data-processing agreements.
- **Data lifecycle & retention:** Classified via **Microsoft Purview**; retention and purge schedules are enforced per data class and client contract (e.g., call recordings, transcripts, candidate records). Training/evaluation datasets are de-identified; PII is redacted before it reaches logs or golden sets.
...
| --- | --- | --- |
| **PCI-DSS** | Payment capture in voice interactions | Pause-and-mask on card capture, PII redaction, network segmentation, CMK encryption, RBAC, audit logging, no card data in prompts/logs. |
...
| **TCPA** | Outbound voice campaigns | Consent capture and verification, do-not-call enforcement, disclosure scripting via Compliance agent, immutable consent audit log. |
...
| **EEOC / NYC Local Law 144** | AI-driven hiring | Bias audits, adverse-impact testing, candidate notice/consent, human-in-the-loop, explainability, model/system cards. |
