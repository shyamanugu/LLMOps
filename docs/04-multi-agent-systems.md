...
Afni's GenAI capability is built on a **multi-agent architecture**: a single **orchestrator/supervisor agent** decomposes each interaction and routes work to a fleet of **specialist agents**, each responsible for one well-scoped capability. This pattern is deliberately reused across both flagship use cases — **Voice AI for contact centers** and **AI-driven HR recruitment** — so that Afni builds, governs, and operates *one* set of agent capabilities rather than two disconnected solutions.
...
| **Knowledge / RAG** | Retrieve grounded answers from Afni policies, KBs, and requisitions via Azure AI Search (hybrid + semantic ranker). Returns citations. |
...
| **Sentiment / Emotion** | Detect frustration, confusion, or escalation cues in real time to trigger tone shifts or handoff. |
| **Escalation / Handoff** | Perform warm transfer to a human with full context and a running summary. |
| **Summarization / QA** | Produce post-interaction summary, disposition coding, and automated QA scoring. |
...
- **Concurrent** — run independent specialists in parallel (e.g., Sentiment and Compliance evaluate the same utterance simultaneously) to protect latency budgets.
...
- **Group-chat** — multiple agents collaborate in a shared thread under the orchestrator's moderation (useful for interview-panel scoring synthesis).
- **Reflection / critic** — a critic agent reviews a draft answer for accuracy, tone, and policy adherence before it is spoken/sent. Improves groundedness on high-stakes replies.
- **Human-in-the-loop (HITL)** — a required approval or decision step for consequential actions (payment commitments, candidate rejections). HITL is mandatory in HR: **AI assists, humans decide.**

## Tool / Function Calling

Specialist agents act on the world through **typed tool definitions** exposed by Azure AI Agent Service. Each tool has a strict JSON schema, is fronted by Azure API Management, authenticates via Entra ID managed identity, and is fully audited. Tools include CRM lookups, payment posting (PCI-scoped), knowledge search, ATS scheduling, and calendar operations. Schema validation on both arguments and results is a deterministic guardrail: malformed or out-of-policy tool calls are rejected before they reach a system of record.
...
| **Long-term (persistent)** | Conversation history, caller/candidate profile, prior dispositions, embeddings | **Azure Cosmos DB** (state + memory), vectors in Cosmos DB / AI Search |

Cosmos DB provides low-latency, globally distributed persistence for agent state and conversation memory, enabling continuity across a caller's multiple contacts or a candidate's multi-stage journey while respecting retention and PII policies enforced via Purview.
...
## One Pattern, Two Use Cases

The identical orchestrator/specialist topology serves both flagships — only the tools, knowledge sources, and policies change:

| Element | Voice AI (Contact Center) | HR Recruitment |
|---|---|---|
| Intent/Router | Caller intent (billing, care, collections) | Candidate intent (apply, status, FAQ) |
| Knowledge/RAG | Client policies, product KBs | Job descriptions, benefits, hiring FAQs |
| Action/Tooling | CRM, billing, payment (PCI) | ATS, calendar, requisition updates |
| Compliance | TCPA, PCI, HIPAA, disclosures | EEOC, NYC LL144, IL AI Video Act, GDPR |
| HITL gate | Payment commitments, escalations | **All hiring decisions — no autonomous rejection** |
| Summarization/QA | Call disposition, QA scoring | Structured-interview scoring (assist), fairness/adverse-impact monitor |

By reusing the pattern, the conversational screening agent in HR literally runs on the same voice platform as the contact-center agents, maximizing reuse of Afni's investment while keeping each domain's guardrails distinct and auditable.
