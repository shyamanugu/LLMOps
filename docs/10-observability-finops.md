...
| --- | --- | --- |
| **Quality** | Groundedness score, relevance, answer correctness (LLM-as-judge + human), task success/containment, thumbs feedback | Azure AI evaluation (online), feedback loop |
...
| **Drift** | Input distribution shift, embedding drift, eval-score decay over time | Eval pipeline, Monitor workbooks |
| **Safety** | Content Safety triggers, prompt-shield (injection) hits, PII-redaction events, guardrail blocks | Content Safety logs, audit trail |
...
- Prompt/response payloads are captured with PII redaction applied before storage; sampling is configurable per risk tier (full capture for Tier 3).
...
Role-based dashboards are published in Azure Monitor workbooks: an **executive/FinOps** view (cost, adoption, outcome KPIs), an **operations** view (latency, errors, escalation), and a **quality/safety** view (groundedness, eval scores, safety triggers).
...
- **Azure API Management** fronts all model traffic as the token-metering gateway, tagging every request with use case, business unit/program, and environment. This enables **showback/chargeback** — allocating cost by use case (Voice AI vs. HR) and by client program, which aligns directly with Gainshare accounting.
- Quotas and rate limits per consumer prevent runaway spend and noisy-neighbor effects.
...
| --- | --- | --- |
| **GPT-4o / gpt-realtime** | Complex reasoning, live voice, high-stakes summarization | Quality and latency where it matters |
| **GPT-4o-mini** | Routing, classification, simple Q&A, high-volume screening | ~10x cheaper; sufficient quality |
| **Open-weight (Llama, Phi)** | Cost-sensitive or on-prem-ish, bulk offline processing | Lower marginal cost, deployment flexibility |

Right-sizing is validated against evaluation gates so cost reductions never silently degrade quality.

### Efficiency Levers

- **Caching:** Semantic/response caching at the gateway and prompt-caching for stable system prompts and retrieved context cut repeated token spend on common queries (FAQs, standard disclosures).
...
- Per-use-case and per-business-unit **budgets** are set in Azure Cost Management with alerts at 50/80/100% of forecast.
...
- A monthly **FinOps review** (part of the CoE cadence) tracks cost per interaction, cache-hit trends, and model-mix, feeding right-sizing decisions back into the platform.
